...
function BrowseApprovedWorkOrder() {
    if (departments !== undefined) {
        for (let i = 0; i < departments.length; i++) {
            window.open(ApprovedWorkOrderURL + WorkOrderID + "/" + departments[i]+".jpg", '_blank');
        }
    ...
    if (WorkOrderID === 0) { alert('يجب إختيار المستخلص'); }
        ...
        if (window.confirm("هل انت متأكد؟")) {
            $.ajax(
                {
                    type: "POST",
                    url: '/WorkOrder/SetWorkOrderApprovedForTechnical',
                    data: {
                        WorkOrderId: WorkOrderID
                    },
                    error: function (result) {
                    ...
                    success: function (result) {
                        if (result === -1) {
                            alert("يجب تأكيد المستخلص السابق أولا");
                        }
                        else if (result !== 0) {
                            updateNotifications();
                            alert("تم التأكيد بنجاح");
...
function getDataForApprovalBtn(workOrderID) {
    $.ajax({
        type: "POST",
        url: '/WorkOrder/getDataForApprovalBtnForTechnical',
        data: {
            WorkOrderId: workOrderID
        },
        error: function (result) {
...
function fillWorkOrderCombobox(reqID) {
    $.ajax({
        type: "POST",
        url: '/WorkOrder/GetWorkOrderApprovedForTechnicalListByRequestID',
        data: {
            RequestId: reqID
        },
        error: function (result) {
