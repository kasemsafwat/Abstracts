...
function OnChangeRequest() {
    ...
    $.ajax({
        type: "POST",
        url: '/Request/GetRequestDataByIDForCompany',
        data: {
            RequestId: $("#RequestNo").val()
        },
...
function browse(code, _FileNameDetails) {
    if (code === undefined) code = $("#hdnRequestCode").val();
    if (FileNameDetails !== "") _FileNameDetails = FileNameDetails;
    window.open(RequestDetailsURL + code + "/" + _FileNameDetails, '_blank');
}
function upload() {
