...
function Search() {
    ...
    $.ajax({
        type: "POST",
        url: '/Request/GetRequestDataByTechnical',
        data: {
        ...
        success: function (result) {
            if (result !== -1) {
                ...
                var gridNotes = $("#gridNotes").data("kendoGrid");
...
function onChangeSelectionRequestDetails(arg) {
    ...
    for (var i = 0; i < selectedRows.length; i++) {
        var dataItem = this.dataItem(selectedRows[i]);
        selectedDataItems.push(dataItem);
        allSelectedIDs.push(dataItem.RequestDetailId);
    }
...
function onChangeSelection2(arg) {
    $("#BrowseApprovedWorkOrder").css('display', 'none');
...
function GetWorkOrderDataForNotifications(RequestID, WorkOrderId) {
    $.ajax({
        type: "POST",
        ...
        success: function (result) {
            ...
            getDataForApprovalBtn(WorkOrderId);
...
function onSelect(e) {
    var upload = $("#ApprovedWorkOrder").data("kendoUpload");
    ...
    FileNameApproved = e.files[0].name;
}
...
function BrowseApprovedWorkOrder() {
    if (WorkOrderCode === "") WorkOrderCode = $("#WorkOrderCode").val();
    if (FileNameApproved === "") FileNameApproved = $("#WorkOrderFileName").val();
    if ($("#workOrderFileApprovedhdn").val() === "") FileNameApproved = $("#workOrderFileApprovedhdn").val();
    window.open(ApprovedWorkOrderURL + workOrderID + "/" + FileNameApproved, '_blank');
}
function browse() {
    if (WorkOrderCode === "") WorkOrderCode = $("#WorkOrderCode").val();
...
function upload() {
    ...
    if (WorkOrderID === 0) {
        ...
        if (FileNameApproved !== null) {
            var upload = $("#FileNameApproved").data("kendoUpload");
            upload.upload();
            //alert('تم الرفع');
            $("#BrowseApprovedWorkOrder").css('display', 'block');
        }
    }
}
function getWorkOrderData(RequestID) {
    $.ajax({
        type: "POST",
        url: '/Request/GetRequestByIDForWorkOrderDataForWorkOrder',
        data: {
            RequestId: RequestID
        },
        error: function (result) {
        ...
        success: function (result) {
            if (result !== -1) {
                ...
                if (result.FileNameApproved === null) {
                    $("#BrowseApprovedWorkOrder").css('display', 'none');
                }
                else {
                    FileNameApproved = result.FileNameApproved;
                    $("#BrowseApprovedWorkOrder").css('display', 'block');
                }
            }
            else {
...
$(document).ready(function () {
    if ($("#request").val() !== null) {
     /*   fillWorkOrderCombobox($("#request").val());*/
        //getWorkOrderData($("#request").val());
        //onChangeRequest($("#request").val());
        //GetWorkOrderDataForNotifications($("#request").val(), $("#workOrder").val())
    }
    if ($("#workOrderFileApprovedhdn").val() !== undefined) {
        FileNameApproved = $("#workOrderFileApprovedhdn").val();
        $("#BrowseApprovedWorkOrder").css('display', 'block');
    }
    ...
    if (WorkOrderID === 0) { alert('يجب إختيار المستخلص'); }
        ...
        if (window.confirm("هل انت متأكد؟")) {
            ...
            if (!validator.validate()) {
                if (fileNotSelected) {
                    alert('يجب إختيار صورة موافقة المكتب الإستشاري')
                }
                return;
            }
            $.ajax(
                {
                    type: "POST",
                    url: '/WorkOrder/SetWorkOrderApprovedForConsultation',
                    data: {
                        WorkOrderId: WorkOrderID
                    },
                    error: function (result) {
...
function getDataForApprovalBtn(workOrderID) {
    $.ajax({
        type: "POST",
        url: '/WorkOrder/getDataForApprovalBtnForConsultation',
        data: {
            WorkOrderId: workOrderID
        },
        error: function (result) {
...
function fillWorkOrderCombobox(reqID) {
    $.ajax({
        type: "POST",
        url: '/WorkOrder/GetWorkOrderApprovedListByRequestIDForConsultation',
        data: {
            RequestId: reqID
        },
        error: function (result) {
        ...
        success: function (result) {
            ...
            RequestID = reqID;
            var combobox1 = $("#WorkOrder").data("kendoComboBox");
            combobox1.select(function (dataItem) {
                return dataItem.WorkOrderId === parseInt($("#workOrder").val());
            });
        }
    });
}
