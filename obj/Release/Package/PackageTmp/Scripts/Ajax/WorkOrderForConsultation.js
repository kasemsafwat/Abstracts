...
function onChangeSelection2(arg) {
    ...
    if (selectedDataItems.length === 1 && selectedDataItems !== []) {
        ...
        gridNotes = $("#gridNotes").data("kendoGrid");
        gridNotes.dataSource.read();
        var requestDetailsGrid = $("#gridRequestDetails").data("kendoGrid");
        requestDetailsGrid.dataSource.read();
...
function getWorkOrderData(RequestID) {
    $.ajax({
        ...
        success: function (result) {
            if (result.WorkOrderId === 0) {
                $("#WorkOrderStartDate").val(result.WorkOrderStartDateString);

            } else if (result === -2) {
                alert('يجب إضافة محضر إستلام الموقع');
                $("#submitbtn").css('display', 'none');
                WorkOrderID = 0;
                $("#Browse").css('display', 'none');
                $("#WorkOrderCode").val('');
                $("#WorkOrderStartDate").dataSource.data('');
                $("#WorkOrderEndDate").val('');
                $("#Note").val('');
                $("#grid").data("kendoGrid").dataSource.data([]);
                $("#gridRequestDetails").data("kendoGrid").dataSource.data([]);
                ...
                $("#Browse").css('display', 'none');
                $("#WorkOrderCode").val('');
                $("#WorkOrderStartDate").val(RequestStartDate);
                $("#WorkOrderEndDate").val('');
                $("#Note").val('');
                $("#grid").data("kendoGrid").dataSource.data([]);
                $("#gridRequestDetails").data("kendoGrid").dataSource.data([]);
...
$(document).ready(function () {
    ...
    $("#requestDetailsfrm").validate({
        ...
        messages: {
            RequestNo:
            {
                required: "من فضلك أدخل رقم أمر الإسناد"
            },
            RequestDate:
            {
            ...
            WorkOrderEndDate:
            {
                required: " من فضلك أدخل تاريخ المستخلص حتي"
            }
        }
    });
    $("#Notefrm").validate({
    ...
    var element1 = document.querySelector('#GridViewNote [aria-label="Select all rows"]');
...
function saveWorkOrder() {
    ...
    var validator = $("#requestDetailsfrm").kendoValidator({
        rules: {
    ...
    $.ajax(
        {
            ...
            success: function (result) {
                if (result === -1)
                    alert("يجب إختيار التاريخ الأعمال حتي بتاريخ بعد الأعمال المنفذه من !");
                else {
                    WorkOrderID = result.WorkOrderId;
                    alert("تم الحفظ بنجاح");
                    $("#submitbtn").css('display', 'block');
    ...
    if (WorkOrderID === 0) {
        ...
        $.ajax({
            ...
            data: {
                WorkOrderDetailId: 0,
                WorkOrderId: WorkOrderID,
                IsStored: $("#stored").prop("checked"),
                RequestDetailIdList: allSelectedIDs,
